...
!pip install pandas geopandas pyreadstat cryptography
```
...
fs.download('projet-hackathon-socstat/data/recensement.sav','recensement.sav')
```

Créer un fichier `encryption_key.key` avec la
clé partagée en amont du hackathon sous la forme

```{.python filename="encryption_key.key"}
laCleDeDechiffrement=
```
...
with open('recensement_encrypted.sav', 'rb') as encrypted_file:
    encrypted_data = encrypted_file.read()
    decrypted_data = cipher_suite.decrypt(encrypted_data)

# Sauvegarde du fichier déchiffré
with open('recensement_decrypted.sav', 'wb') as decrypted_file:
    decrypted_file.write(decrypted_data)

print("Le fichier a été déchiffré.")
```

## `R` {.unnumbered .unlisted}

Il est nécessaire de déchiffrer les données avec `Python`
